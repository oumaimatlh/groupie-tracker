...
STOP => ne lit pas le body => Pourquoi ??
    Le body est apres les headers
    il peut etre 
        tres gros , envoye lentement... 
        donc go ne peut pas bloquer jusqu'z ce que tout le body arrive 
        -> C'est un lecteur branché directement sur le socket TCP 
        donc CONNEXION TCP RESTE OUVERTE CAR BODY N A PAS ENCORE ETE CONSOMME 


    
#### Interface

si tu sais faire ces actions (methodes) alors tu es accepté 

interface =>>> regle 

pr exemple si tu est un ReadCloser tu dois Read et Close  => contient des comportement diff mais pour implementer cette interface 

C/c ce que doit savoir faire un type

un exemple 
    type Afficheur interface {
        Afficher()
    }
    ...
    a = STR("oumaima")
    a.Afficher() tu dois m'afficher =>>> je suis un string 
